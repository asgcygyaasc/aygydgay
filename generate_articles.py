...
def make_article(i, keyword):
    parts = [TG, "", f"# {make_title(i)}", "", f"## 一、【{keyword}】基础说明", ""]
    seed = (i * 7) % len(PATTERNS)
    for n in range(12):
        p = PATTERNS[(seed + n) % len(PATTERNS)].replace("【关键词】", f"【{keyword}】")
        parts += [p, "", f"### 第{n+1}点：实际使用中的关注事项", "", f"第{n+1}点主要围绕【{keyword}】展开。实际操作时，应先确认账号属于本人正常使用范围，再检查登录环境、绑定信息和安全提醒。对于陌生链接、非官方页面以及要求提供验证码的请求，应保持谨慎，避免泄露账号信息。", ""]
    parts += ["## 二、日常维护建议", "", f"完成【{keyword}】相关设置后，建议定期检查登录设备、隐私权限和绑定资料。遇到异常登录、频繁验证或无法正常使用等情况，应优先查看微信客户端提供的提示，并按照官方流程处理。", "", "## 三、总结", "", f"总体来看，【{keyword}】的重点不是追求复杂操作，而是建立清晰、稳定和安全的使用习惯。", ""]
    return "\n".join(parts)
...
def main():
    ...
    kws = load_keywords()
    for i in range(900):
        (OUT / f"{i+1:04d}.md").write_text(make_article(i, kws[i % len(kws)]), encoding="utf-8")
